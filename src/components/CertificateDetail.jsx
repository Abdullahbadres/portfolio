...
const CertificateDetails = () => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0)
    const selectedCertificate = getCertificateById(id)

    if (selectedCertificate) {
      setCertificate(selectedCertificate)
      document.title = `${selectedCertificate.title} | Abdullah Badres`
    } else {
      setCertificate(null)
      document.title = "Certificate Not Found | Abdullah Badres"
    }
  }, [id])
    ...
    return (
      <div className="min-h-screen bg-[#030014] flex items-center justify-center px-4">
        <div className="text-center space-y-6">
          <h2 className="text-xl md:text-3xl font-bold text-white">Certificate Not Found</h2>
          <p className="text-gray-400">This certificate does not exist or may have been removed.</p>
          <button
            onClick={() => navigate("/")}
